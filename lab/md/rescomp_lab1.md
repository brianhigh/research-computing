...
For each
of the downloads below, you *could* download "binaries" (and any dependencies) 
in a zip file and extract to, e.g., U:\\bin. You would need to create this
"bin" folder first. After extraction of the ZIP archives, you would move all 
of the files you have extracted directly into the "bin" folder. However, there
is an easier way -- described next, so *don't* actually bother doing all of 
this. (Plus these versions are rather old, anyway.)

-   diff
    ([Windows](http://gnuwin32.sourceforge.net/packages/diffutils.htm))
    - a standard text comparison tool

-   find
    ([Windows](http://gnuwin32.sourceforge.net/packages/findutils.htm))
    - a standard file search tool

-   grep ([Windows](http://gnuwin32.sourceforge.net/packages/grep.htm))
    - a standard text search tool

-   GNU CoreUtils
    ([Windows](http://gnuwin32.sourceforge.net/packages/coreutils.htm))
    - a collection of standard file, text, and shell utilities commonly
    found on Mac OSX, Unix and Linux systems
    
*Instead* of doing that, you can get *all* of these tools by simply installing:

-    Rtools
     ([Windows](https://cran.r-project.org/bin/windows/Rtools/)) - a collection
     of standard utilities, including many GNU CoreUtils, diff, find, and grep,
     plus software compiling tools
...
#### **D. Apps to install as needed... (not today unless you have extra time)**
...
#### **A. Commonly Used Command-Line Utilities**
